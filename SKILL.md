...
- **`/kimiflow --resume <slug>`** — continue: read `.kimiflow/<slug>/STATE.md`, resume at the first open phase. No re-clarification — the spec files suffice. Without `<slug>` → list existing `.kimiflow/*/` with each run's `STATE.md` `Status:` (absent → `active`), so deliberately-parked **backlog** items (a finished, plan-gate-approved plan awaiting implementation) are visible as a backlog, and ask.
...
3. **Resume check.** With `--resume <slug>`: read its `STATE.md`, resume at the first unfinished phase. Else if `.kimiflow/<slug>/STATE.md` exists → continue. Else create `STATE.md` (feature/problem, slug, date, mode, scope tier, a `Status:` line — `active` while the run is in progress, `backlog` once a complete plan is deliberately parked before implementation (phases 0–4 done, 5 open); an absent `Status:` reads as `active` — and one status line per phase `Phase 0..7`). The `backlog` marker is set ONLY by a Phase-4 pre-build park reaching 0–4-done (step 6/7); an earlier stop (Explore, mid-phase) stays `active`.
...
8. **Project Map Bootstrap check** (recommended, never a prerequisite; full rules → reference.md "Project Map Bootstrap"). If the invocation is `--project-map quick|standard|deep`, create/update `.kimiflow/project/` at that depth, report paths, then STOP. If it is `--project-map skip`, record `project_map: skipped` in `STATE.md` and continue normally. Otherwise, on non-trivial runs, if `.kimiflow/project/INDEX.json` is missing, offer once: `standard` (recommended) / `quick` / `deep` / `skip`. Decline, headless, no answer, or `skip` → continue unchanged. If accepted, run the bootstrap before Phase 2 so later understanding can reuse the map.
