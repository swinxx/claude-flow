...
| `learning_lifecycle_json` / `learning_usefulness_json` non-object usage (`_usage_items`) | a valid non-object `MEMORY-USAGE.json` top level (Bash passes `jq -e .` then errors on `.items`), or a `false`/number/array usage ENTRY (jq errors on e.g. `false.use_count`), makes the Bash function emit empty output | `_usage_items` falls back to `{}` for any non-object top level / `.items`, and a non-object entry yields `use_count` 0 | Unreachable: `MEMORY-USAGE.json` is always an object whose `.items` is an object of item-objects. The port stays robust (valid result) where Bash crashes; same class as the `usage_summary_json` non-object row. All reachable usage shapes (object / missing / null / false / `{}` / items-null) match byte-for-byte. |
| `provider_detection_json` / `provider_auth_json` curl gate | `command -v curl` absent -> detection returns `status:"unavailable", reason:"curl_unavailable"`; the auth token probe is blocked with `probe_blocked_reason:"curl_unavailable"` | the port always probes via stdlib `urllib`, so it never emits the `curl_unavailable` shape/reason | Generalizes the jq/sqlite stdlib rows: `urllib` is always importable, so the curl-absent branch is unreachable. On any host with curl (the harness host) there is no diff. |
| `provider.py` TLS verification + redirects (`_http_probe` / `_ssl_context` / `_NoRedirect`) | `curl -k` (no `-L`) skips TLS verification for the local self-signed Obsidian REST API and does NOT follow redirects | `urllib` with `ssl.CERT_NONE` (verification disabled) + a `_NoRedirect` opener so 3xx is terminal | **Replicated, not a divergence.** Faithful to `curl -k`; the probe targets ONLY a `_normalize_loopback_origin`-validated loopback host AND never follows a redirect, so the bearer token never leaves the machine (two compensating controls for the disabled verification). Default urllib WOULD follow a 3xx and re-send `Authorization` cross-origin (token leak); `_NoRedirect` prevents that and restores `curl`'s 3xx -> `token_unverified` parity. Flagged by the security hook; intentional for the local self-signed cert. |
| `cmd_curate` MEMORY-INDEX.json write | Bash `jq . > "$index"` is a plain truncating redirect that follows a symlink | the port uses `store.atomic_write` (temp file + atomic rename; refuses to write THROUGH a symlink) | Unreachable: `MEMORY-INDEX.json` is a regular file. The port matches the rest of the codebase's atomic writers and is the safer behavior (atomic + no symlink-follow); the written content is byte-identical (pretty 2-space + trailing newline). |
| `status_json` sqlite_available (`command -v sqlite3`) | Bash sets `sqlite_available` from the sqlite3 CLI's presence; it gates `recall_index.sqlite_available` + the `recall_index_missing` curation reason | the port uses `recall_index.fts5_available()` (stdlib module FTS5 probe) | Generalizes the recall-engine row: the port never shells to the sqlite3 CLI, so it probes the actual capability it needs (FTS5). Both true on supported hosts (CLI present + module has FTS5), so no diff; the port is stricter (an FTS5-less build reports false, which is the correct gate for building the recall index). |
| `provider_sync_base_candidates_json` non-list evidence/fingerprints | a non-list `evidence`/`evidence_fingerprints` or a non-object fingerprint makes jq's `any`/`all` error, aborting the whole `map` -> the function drops the entire candidate batch (empty) | `_sync_base_candidates` guards with `isinstance` and skips just the offending row | Unreachable: schema-valid learnings always carry list `evidence` and an array of fingerprint objects. The port is the safer behavior (one bad row excluded, not the whole batch); same robustness class as the usage rows. |
| `provider_manifest_json` non-object / `provider_detection_json` timeout | Bash `jq '.'` returns a valid non-object manifest verbatim (callers then error on `.type`); a non-numeric `KIMIFLOW_OBSIDIAN_DETECT_TIMEOUT` is passed to curl, which errors -> probe fails | `manifest_json` returns the default manifest for any non-dict; `_detect_timeout` falls back to `0.35` for a non-numeric value | Unreachable: `VAULT-PROVIDER.json` is always an object and the timeout is a numeric config. The port stays robust; reachable manifest/timeout inputs match byte-for-byte. |
| `recall` `jsonl_hits` / `run_artifact_hits_json` / `update_usage_metrics` non-object rows | a non-object JSONL line (jq `.status`/field access), a non-object recall hit, or a non-object `MEMORY-USAGE.json` makes jq error and abort the function | the port skips a non-dict JSONL/hit row and treats a non-dict `MEMORY-USAGE.json` as the default shape | Unreachable: JSONL rows, recall hits, and `MEMORY-USAGE.json` are always objects. Same robustness class as the existing usage/summary non-object rows; the port stays valid where Bash would crash. All reachable shapes match byte-for-byte. |
| `recall --write` writers + run-artifact summary `cut -c1-420` | RECALL.md/.json use `> "$path"` (truncating redirect, follows a symlink); `update_usage_metrics` uses `mktemp`(0600)+`mv` (atomic, replaces a symlinked path); the run-artifact summary truncates with `cut -c1-420` (byte-based under the C locale) | RECALL.md/.json go through `store.atomic_write` (temp+rename, refuses to write THROUGH a symlink); MEMORY-USAGE.json uses `atomic_write(mode=0o600, refuse_symlink=False)` to match `mktemp`+`mv`; the summary truncates by codepoint `[:420]` | Generalizes the `cmd_curate` MEMORY-INDEX row: the RECALL.md/.json paths are never symlinks (unreachable) and the port is atomic+safer; MEMORY-USAGE.json is **replicated** (same temp dir, 0600, symlink-replacing rename). The `[:420]` codepoint slice matches the shipped `slugify` `cut -c` convention; only a multibyte summary crossing the 420 boundary differs (unreachable for ASCII run-artifacts). The stdout recall object carries NO timestamp; only the written RECALL.md (`Generated:`) and MEMORY-USAGE.json (`updated_at`/`at`/`last_used_at`) embed `iso_now`. |
| `resolve_run_dir` die-in-`$()` (verify-run / review-run) | every caller does `run_dir="$(resolve_run_dir ...)"`, so the helper's `die "run path required"/"run directory not found" 2` runs in a **subshell**: the message hits stderr but the exit code is **discarded** and the caller gets an **empty** `run_dir`, then limps on (`review="$run_dir/LEARNING-REVIEW.md"` -> `/LEARNING-REVIEW.md` -> `missing_review` exit 1) | `resolve_run_dir` writes the same `die` line to stderr and **returns `""`** (no clean exit 2); the caller builds the review path with string concat, reproducing `/LEARNING-REVIEW.md` and the `missing_review` exit-1 path | **Replicated, not a divergence.** A clean exit-2 (the `die`'s declared intent) would have been "more correct", but the Bash command-substitution swallows it, and a drop-in replacement must match the real exit code + stdout. Caught by byte-for-byte grounding (stdout + stderr + exit all match) after an initial `DieError`/exit-2 design was rejected. The same helper feeds review-run, which inherits the identical empty-`run_dir` behaviour. |
